...
export default function ClientPackagesPage() {
  ...
  return (
    <AdminMembershipsShell title="Sell / use visit packs">
      <p className="mb-4 text-sm text-zinc-600">
        Give a client a visit pack, then use <strong>Use 1 visit</strong> when they come in (or restore
        if you undo). Need a pack product first?{' '}
        <a href="/admin/memberships/packages" className="font-medium underline">
      ...
      <div className="mb-4">
        ...
        <form onSubmit={handleAssign} className="flex flex-wrap gap-2">
          ...
          <Field label="Visit pack">
            <select className={inputClass} value={productId} onChange={(e) => setProductId(e.target.value)} required>
              <option value="">Select visit pack</option>
              {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </Field>
          <div className="flex items-end"><Button type="submit">Sell pack</Button></div>
        </form>
        </Card>
      </div>
      {loading ? <LoadingState /> : (
        <Card title="Visits left">
          <table className="w-full text-left text-sm">
            <thead><tr className="border-b text-zinc-500"><th className="py-2">Client</th><th>Pack</th><th>Remaining</th><th>Status</th><th>Actions</th></tr></thead>
            <tbody>
              {packages.map((p) => (
                <tr key={p.id} className="border-b border-zinc-100">
                  ...
                  <td className="flex gap-2">
                    {p.status === 'active' && p.quantity_remaining > 0 ? (
                      <button type="button" className="text-xs underline" onClick={() => redeemClientPackage(p.id, 1).then(load)}>Use 1 visit</button>
                    ) : null}
                    ...
                    {p.status === 'active' && p.quantity_remaining <= 0 ? (
                      <span className="text-xs text-amber-700">Empty — sell again</span>
                    ) : null}
                  </td>
                </tr>
              ))}
