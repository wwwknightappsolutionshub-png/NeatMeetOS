...
class RebookingAutomationService
{
    ...
    public function generate(array $options = []): array
    {
        ...
        foreach ($campaigns as $campaign) {
            $runs[] = $this->generateForCampaign($campaign, $windowDays, $runSource, $options);
        }
    ...
    private function generateForCampaign(
        ...
        return DB::transaction(function () use ($campaign, $windowDays, $runSource, $options) {
            ...
            $run = MarketingRun::query()->create([
                ...
                'trigger_type' => MarketingTriggerType::REBOOKING_NUDGE,
                'run_source' => $runSource,
                'status' => MarketingRunStatus::PROCESSING,
                'filters_json' => [
                    'rebooking_window_days' => $windowDays,
                ...
                ->where(function ($q) use ($cutoff) {
                    $q->where('ends_at', '<=', $cutoff)
                        ->orWhere(function ($inner) use ($cutoff) {
                            $inner->whereNull('ends_at')->where('starts_at', '<=', $cutoff);
                        });
                })
            ...
            foreach ($appointments as $appointment) {
                ...
                if ($client === null || isset($processedClients[$client->id])) {
                    continue;
                }

                $processedClients[$client->id] = true;

                if (in_array($client->id, $futureClientIds, true)) {
                    continue;
                }

                if ($this->alreadyQueued($client->id)) {
                    continue;
                }
                ...
                $template = $campaign->template
                    ?? $this->templateService->resolveForTrigger(
                        MarketingTriggerType::REBOOKING_NUDGE,
                        $campaign->channel,
                ...
                MarketingMessage::query()->create([
                    ...
                    'purpose' => MarketingMessagePurpose::REBOOKING_NUDGE,
            ...
            $run->summary_json = [
                'clients_matched' => count($processedClients),
                'eligible' => $eligible,
                'skipped' => $skipped,
                'by_reason' => $byReason,
            ];
    ...
    private function alreadyQueued(string $clientId): bool
    {
        return MarketingMessage::query()
            ...
            ->where('purpose', MarketingMessagePurpose::REBOOKING_NUDGE)
